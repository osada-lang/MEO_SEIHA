...
app.listen(port, () => {
  ...
  setTimeout(async () => {
    try {
      ...
      // Cascade delete existing records for 'thanx-create-uuid' to completely delete the account and purge old state
      const targetThanxId = 'thanx-create-uuid';
      console.log(`🧹 Deleting and purging THANX CREATE OWNER data from live database for ID: ${targetThanxId}...`);
      ...
      console.log('🧹 "合同会社THANX CREATE" OWNER account completely deleted and purged from the database.');
