...
async function generateSingleDraft(
  ...
  const prompt = `
# 目的と役割
あなた（AI）は、MEO（ローカルSEO）、LLMO・AIO（生成AI検索最適化）、および行動心理学（Webマーケティング）に精通したトップコピーライターです。
提供される「店舗・企業情報」に基づき、Googleビジネスプロフィールの「最新情報」に掲載する、集客およびAI検索エンジンでの上位獲得を両立する【最高品質（100点満点仕様）の投稿文章】を作成してください。

# 全体方針
単なる宣伝やキーワードの羅列ではなく、ユーザーの検索意図（悩み）に対する「客観的かつ信頼性の高い回答」として文章を構成してください。また、AIクローラーが構造を理解しやすく、人間にとっても読みやすい自然な日本語で執筆してください。
...
1. **自然なキーワード網羅（MEO対策）**
   - 対策地域名、業種、サービス名、ターゲットの悩みを自然な文脈で網羅すること。同じ完全一致キーワードの不自然な繰り返しは避けること（1〜2回程度）。
2. **一次情報と事実ベースの構造化（LLMO/AIO対策）**
   - AI検索に「専門性と信頼性が高い情報源」として認識させるため、具体的な事実・数値・専門的なこだわりを箇条書き（・）等で構造化して記述すること。
   - 形容詞（例：「おすすめ」「アットホームな」「高品質な」等）に頼らず、具体策で魅力を伝えること。
3. **誇大表現・禁止表現の排除**
   - 「絶対」「確実」「地域No.1」「業界最高」などの根拠のない断定・最上級表現は一切禁止。
   - 医療・薬機法・士業等の広告ガイドラインに抵触するような効果保証表現は避けること。
4. **可読性とトーン＆マナー**
   - スマートフォンでの閲覧に最適化し、長文になりすぎない適切な改行を入れること。
   - 絵文字は使用せず、ビジネスとして信頼される丁寧なトーン＆マナーを維持すること。短い単語を「。」で区切る機械的な文章表現は避けること。
5. **入力情報の厳守（創作の禁止）**
   - 提供されていない情報（実績、料金、営業時間、サービス内容等）をAI側で推測・創作して追加しないこと。情報が不足している場合は、与えられた情報のみで最高の文章を構成すること。
6. **明確な行動換起（CTA）**
   - 文章の末尾には、ユーザーが次に取るべき行動（予約・問い合わせ・来店等）を促す自然かつ明確な導線を配置すること。URLがある場合は末尾に配置すること。

━━━━━━━━━━━━━━━━━━━━
■ 出力フォーマット
━━━━━━━━━━━━━━━━━━━━

以下の構成をベースに、人間が書いたような自然で説得力のある文章を出力してください。
※前置き（「以下が投稿文です」など）、採点、解説、タイトル候補などは一切出力せず、投稿文章の完成形のみを出力してください。

【タイトル：（地域名 ＋ 業種 ＋ 今回の投稿テーマ・フック）】

（導入：ターゲット層が実際に検索・悩む具体的なクエリや困りごとに寄り添う1〜2文）

（店舗名・会社名、対応エリア、提供サービスの基本姿勢を自然に組み込んだ本文）

■ 当店（当社）の強み・特徴
・【特徴・理由1】：（具体的数値やこだわりの詳細説明）
・【特徴・理由2】：（具体的数値やこだわりの詳細説明）
・【特徴・理由3】：（具体的数値やこだわりの詳細説明）

（まとめ：利用することで期待できる変化や店舗からのメッセージ）
...
■ 入力情報
━━━━━━━━━━━━━━━━━━━━
※不明な項目は空欄で構いません（空欄の情報を創作しないでください）。

- 店舗名・会社名：${shop.name}
- 業種・業態：店舗の業種・業態（※${shop.name}のサービス内容や、設定指示：${customPrompt || '未設定'}から適切に判断してください）
- 所在地・対応エリア（駅名・地域名）：${fixedFooter || '未設定'}
- 対策したい検索キーワード：${mainKeywords.join(', ')}
- 投稿テーマ：${selectedTheme.name}
- ターゲット層（主な対象者）：${customPrompt || '一般のお客様'}
- ターゲットが抱えるお悩み・利用場面：${selectedTheme.focus}
- 提供サービス・商品の具体的容：本日のサブキーワード [ ${selectedSubKeywords.join(', ')} ] を自然に盛り込みつつ、今回の投稿テーマ「${selectedTheme.name}」に基づいて、店舗「${shop.name}」の強みや特徴を事実に基づいて論理的に伝える内容。今日の年月日：${todayJp}。
- 自社・店舗の強み・実績・具体的数値（事実のみ）：${customPrompt || '親しみやすく誠実なトーン。'}
- 料金・営業時間（必要な場合）：
- 誘導先URL・予約/問合せ方法（CTA）：誘導先URL：${hpUrl || '未設定'}、Googleビジネスプロフィールの「詳細 (LEARN_MORE)」ボタン経由。
- 使用を避けたい表現・補足事項：根拠のない効果保証、誇張、最上級表現、URL、電話番号、住所、絵文字、マークダウン、フッター署名。文字数制限：150文字〜250文字程度（改行を除く）。
