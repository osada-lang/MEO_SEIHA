...
async function generateSingleDraft(
  ...
  const prompt = `
# 目的
あなた（AI）は、Googleビジネスプロフィール（GBP）運用、ローカルSEO（MEO対策）、および生成AI検索最適化（AIO/LLMO）に熟知した、プロのWebマーケター兼コピーライターです。
指定された「店舗・企業情報」をもとに、Googleの検索エンジン・マップ・生成AIすべてが「どこで、誰が、何を提供しているのか」を完璧に理解し、ターゲットの行動を促す【100点満点のGBP最新情報 投稿文章】を1本作成してください。

# 満たすべき「100点」の条件（評価基準）
1. 冒頭のフック：ターゲット層が実際に検索や質問をしそうな「具体的かつ切実なお悩み（検索意図）」への問いかけから始める。ただし、機械的な定型文や毎回同じ書き出しは避ける。
2. 地域・業種の最適化（MEO）：ターゲット地域、主要駅、業種名、具体的なサービス内容を文章の流れに合わせて自然に盛り込む。キーワードの不自然な羅列や繰り返しは厳禁とし、完全一致キーワードは原則1〜2回に抑える。
3. AI検索への構造化（AIO/LLMO）：AIクローラーが「専門性と客観的信頼性が高い一次情報」と判断できるよう、具体的な事実、特徴、数値、専門用語を「・」を用いた箇条書きで構造的に整理する。
4. 誇大表現の排除（信頼性担保）：「絶対」「必ず」「確実」「地域No.1」など、根拠のない断定・最上級表現は一切使用しない。形容詞（例：アットホームな、おすすめの）に頼らず、事実ベースで強みを書く。
5. 広告ガイドラインの遵守：医療、美容、健康、士業、金融、不動産など、表現規制の厳しい業種の場合、効果保証や誤認を招く表現を徹底的に避ける。
6. 可読性と自然な日本語：スマートフォンでの視認性を重視し、適切な位置で改行を入れる。見出しや記号（絵文字含む）を多用せず、人間が書いた自然な日本語にする。ハッシュタグは原則使用しない。
7. 情報の厳格性（創作の禁止）：入力情報にない事実（数値、実績、料金、営業時間、サービス内容など）を推測・創作して追加することを固く禁じる。
8. 明確な行動喚起（CTA）：投稿の最後に、今回の投稿目的に合致した次のアクション（予約、問い合わせ、詳細確認など）を不自然に埋め込まず、最後の行へ綺麗に案内する。

# 入力情報
※不明な項目は空欄（または未指定）で構いません。空欄の情報を推測・創作しないでください。
* 店舗名・会社名：${shop.name}
* 業種・業態：店舗の業種・業態（※${shop.name}のサービス内容や設定指示：${customPrompt || '未設定'}から適切に判断してください）
* 所在地・対応エリア：${fixedFooter || '未設定'}
* 対策したい地域名・主要駅名：${fixedFooter ? '所在地フッターから抽出される正しい地域・主要駅名' : '未設定'}
* ターゲット層（主な対象者）：${customPrompt || '一般のお客様'}
* 対象者が抱えている具体的な悩み・利用場面：${selectedTheme.focus}
* 主な商品・サービス（今回紹介するもの）：${mainKeywords.join(', ')}
* 自社・店舗の具体的な強みや数値実績（事実のみ）：${customPrompt || '親しみやすく誠実なトーン。'}
* 料金・営業時間（記載する場合）：
* 誘導先URL・希望する行動（CTA）：誘導先URL：${hpUrl || '未設定'}、Googleビジネスプロフィールの「詳細 (LEARN_MORE)」ボタン経由。本日のサブキーワード [ ${selectedSubKeywords.join(', ')} ] も盛り込む。年月日：${todayJp}。

# 出力フォーマット
以下の構成に従って、600〜900文字程度（同じ意味の言い換えによる水増しは不可）で文章を作成してください。
タイトル候補、採点、解説、前置き（例：「以下が投稿文です」など）は一切出力せず、**完成した投稿文章だけ**を出力してください。

【地名＋業種＋店舗名やフックを組み合わせた、記号や絵文字のない自然な見出し】

（ユーザーのお悩みや利用場面に寄り添う導入文）

（店舗・企業の基本紹介と、地域名・業種・サービス内容の自然な連動）

■ 私たちが選ばれる3つの特徴
・【特徴1】：（具体的な理由、事実、工夫、専門的な補足など）
・【特徴2】：（具体的な理由、事実、工夫、専門的な補足など）
・【特徴3】：（具体的な理由、事実、工夫、専門的な補足など）

（ターゲットにとってのメリットや、利用する際の具体的な判断材料の提示）

（今回の投稿テーマに関する補足や、利用条件・営業時間などの事実情報）
