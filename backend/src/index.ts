...
app.listen(port, () => {
  ...
  setTimeout(async () => {
    try {
      ...
      // 🧹 Cascade delete on-request for Agency A, B, and X from live DB
      console.log('🧹 Cascading deletion for Agency A, B, X and their stores on request...');
      const targetAgencies = ['代理店A', '代理店B', '代理店X'];
      for (const agencyName of targetAgencies) {
        // Find all OWNER shops belonging to this agency
        const childShops = await prisma.shop.findMany({
          where: { agency_name: agencyName }
        });
        for (const shop of childShops) {
          console.log(`🧹 Purging child shop of ${agencyName}: ${shop.name} (${shop.email})`);
          await prisma.replyTemplates.deleteMany({ where: { shop_id: shop.id } });
          await prisma.shopKeywords.deleteMany({ where: { shop_id: shop.id } });
          await prisma.reviewLogs.deleteMany({ where: { shop_id: shop.id } });
          await prisma.magicLinkToken.deleteMany({ where: { shop_id: shop.id } });
          await prisma.shop.deleteMany({ where: { id: shop.id } });
        }
        // Delete the agency accounts themselves if they exist
        const agencyAccounts = await prisma.shop.findMany({
          where: { name: agencyName, role: 'AGENCY' }
        });
        for (const agency of agencyAccounts) {
          console.log(`🧹 Purging agency account: ${agency.name} (${agency.email})`);
          await prisma.replyTemplates.deleteMany({ where: { shop_id: agency.id } });
          await prisma.shopKeywords.deleteMany({ where: { shop_id: agency.id } });
          await prisma.reviewLogs.deleteMany({ where: { shop_id: agency.id } });
          await prisma.magicLinkToken.deleteMany({ where: { shop_id: agency.id } });
          await prisma.shop.deleteMany({ where: { id: agency.id } });
        }
      }

      // Also specifically delete the static demo IDs if they exist
      const targetAgencyId = 'demo-agency-uuid';
      const targetAvenirId = 'demo-store-uuid';
      console.log('🧹 Purging explicit demo IDs from live database...');
      ...
      const meoseihaEmails = ['meoseiha@dairiten.x', 'meoseiha@avenir'];
      for (const email of meoseihaEmails) {
        const found = await prisma.shop.findUnique({ where: { email } });
        if (found) {
          console.log(`🧹 Purging mock account by email: ${found.name} (${found.email})`);
          await prisma.replyTemplates.deleteMany({ where: { shop_id: found.id } });
          await prisma.shopKeywords.deleteMany({ where: { shop_id: found.id } });
          await prisma.reviewLogs.deleteMany({ where: { shop_id: found.id } });
          await prisma.magicLinkToken.deleteMany({ where: { shop_id: found.id } });
          await prisma.shop.deleteMany({ where: { id: found.id } });
        }
      }
      console.log('🧹 Cascade delete finished successfully.');
      ...
      const newShops = [
        ...
        },
        {
          ...
          custom_prompt: '名古屋 大橋ボクシングジムの魅力（本格ボクシング指導、フィットネス、ダイエット、初心者歓迎、プロ育成、楽しいトレーニング環境）を情熱的かつ誠実にお伝えください。',
