...
- **処理フロー**:
  ...
  3. 毎日指定時間に、ストックされた画像から自動で1枚選択し、設定された「メインキーワード」および「サブキーワード（プールからランダム抽出）」に加え、店舗ごとに設定された固有のAI指示文（`custom_prompt`）を反映した投稿テキストをAIで生成します。これにより、業種（美容院、ラーメン屋など）や店舗の強みに完全に適応した訴求文を作成できます。
...
flowchart TD
    ...
    RatingCheck -- "星3〜5 (高・中評価)" --> AutoSend["人間味を出すため1時間待機し、\n事前登録の5つからランダムに自動送信\n【AI不使用：APIコスト0円】"]
...
- **自動返信（星3〜5）**:
  - **人間味を出すための1時間遅延返信仕様（新規追加仕様）**:
    - 口コミ検知後、botっぽさを完全に排除して「店舗人間が自ら返信した温かみ」を演出するため、**即座には送信せず「1時間後」に自動送信**します。
    - 検知した口コミデータは、データベース上でステータスを「送信待ち（pending_reply）」、送信予定時刻を「検知時刻＋1時間」として保存。定期実行バッチ（cron）がこの時刻の到来を検知して自動で送信します（初動1回のみ、AI API費用は一切発生しません）。
- **低評価対応（星1〜2）**:
...
- `custom_review_prompt` (店舗固有の低評価お詫びAI生成追加指示・トーン指定。Nullable)
- `created_at`

### 4.4. ShopKeywords (店舗キーワード設定テーブル)
毎日自動投稿に使用するキーワードや店舗固有のプロンプトを管理します。
- `id` (PK)
- `shop_id` (FK)
- `main_keywords` (メインキーワード配列: 最大5単語)
- `sub_keywords` (サブキーワード配列: 最大15単語)
- `fixed_footer` (固定署名テキスト)
- `custom_prompt` (店舗固有の自動投稿文AI生成追加指示・トーン＆アピールポイント指定。Nullable)
- `updated_at`
...
- `status` (状態: "sent"＝送信完了、"draft"＝保留下書き、"pending_reply"＝送信待ち、"failed"＝送信エラー)
- `send_after` (送信予定日時：自動遅延返信の判定用。初期値：検知日時＋1時間)
- `created_at`
