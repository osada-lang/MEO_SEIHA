...
365MEOシステムは、店舗事業者のGoogleビジネスプロフィール（GBP）の運用（日々の写真自動投稿、口コミ自動返信、低評価検知アラート）を、Google Drive連携およびAI/定型エンジンを用いて効率化・自動化するSaaS型マルチテナントシステムです。
...
2. **営業・運用担当者（365 / THANX）**
   ...
   - クライアントごとに異なるAI APIキーの割り当てや管理を行う。
3. **システム（365MEO サーバー）**
...
システムは、プログラム本体を1つの共同サーバーで稼働させ、データベース内でテナント（365 / THANX）や各店舗のデータを切り替える「マルチテナント方式」を採用します。AIのAPIキーやGBP API認証情報はテナント・店舗ごとにデータベースに保持されます。
...
graph TD
    ...
    %% システムサーバー
    subgraph SystemServer ["365MEO システムサーバー (共同サーバー)"]
        API["Web / API サーバー"]
        Cron["定期実行バッチ (Cron)"]
        LLM["AI連携モジュール"]
...
- **フォールバック（画像なし対策）**:
  - 新規画像がない場合は、オンボーディング時に初期登録された「看板・店舗写真」などのストック画像からシステムがランダムに使い回して自動投稿を行います。
- **運用ルール【合意済】**:
...
マルチテナント（365 / THANX）を単一のデータベースで安全に管理するためのテーブル構成案です。

### 4.1. Tenants (テナントテーブル)
システムを利用する販売元ブランドを管理します。
- `id` (PK)
- `name` (ブランド名: "365meo" または "Thanxcreate")
- `ai_provider` (利用するAIサービス名: "gemini" または "openai")
- `ai_api_key` (各社が用意したAIのAPIキー)
...
- **連携方式**: サービスアカウント認証
- **フォルダセキュリティ**:
  - システム所有のGoogle Cloudプロジェクト内に「サービスアカウント（システム専用メールアドレス）」を発行します。
  - 各店舗ごとにシステム側で専用フォルダを作成し、そのフォルダのみの「編集権限」を店舗オーナーのGoogleアカウントに共有します。
  - これにより、店舗オーナーは**自分用の専用フォルダしか閲覧できず、他店の画像は完全に遮断**されます。
  - システム側はサービスアカウントを用いて、API経由で各フォルダの画像を定期的に自動スキャン・同期します。
